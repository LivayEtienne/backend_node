...
exports.addProgram = async (req, res) => {
    try {
      ...
      // Vérifiez que `programmes` est un tableau
      if (!Array.isArray(plant.programmes)) {
        plant.programmes = [];
      }
      ...
      // Associer le programme à la plante
      plant.programmes.push(newProgram); // Ajoute le programme dans le tableau de programmes de la plante
  
      // Sauvegarder la plante avec le programme ajouté
      await plant.save();
      res.status(201).json({ message: 'Programmation enregistrée avec succès !' });
    } catch (error) {
...
exports.getPrograms = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erreur lors de la récupération des programmes', details: error });
  }
};
...
exports.getProgramById = async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Erreur lors de la récupération de la programmation', details: error });
    }
  };
...
exports.deleteProgram = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erreur lors de la suppression', details: error });
  }
};
